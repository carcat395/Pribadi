...
public class ResultScreen : MonoBehaviour
{
    ...
    private void UpdateResult(int[] scores, Dictionary<string, int> itemResults)
    {
        ...
        if(scores[0] < (GameManager.instance.globalExtraversionAverage * GameManager.instance.globalAverageCount) * 0.75f)
        {
            _view.resultDescriptions[0].text = content["0"] + content[new string(0 + "_Low")];
        }
        else if(scores[0] > (GameManager.instance.globalExtraversionAverage * GameManager.instance.globalAverageCount) * 0.75f && scores[0] < (GameManager.instance.globalExtraversionAverage * GameManager.instance.globalAverageCount) * 1.25f)
        {
            _view.resultDescriptions[0].text = content["0"] + content[new string(0 + "_Mid")];
        }
        else if(scores[0] > (GameManager.instance.globalExtraversionAverage * GameManager.instance.globalAverageCount) * 1.25f)
        {
